...
// Componente para os filtros (pode ser movido para um arquivo separado)
function FilterSidebar({ isMobile = false }) {
  ...
  const marcasUnicas = [...new Set(todosEquipamentos.map((item) => item.marca))]
    ...
    <div className="space-y-4">
      <Accordion type="single" collapsible>
        ...
        <AccordionItem value="preco">
          <AccordionTrigger>Preço Mensal</AccordionTrigger>
          <AccordionContent>
            <div className="space-y-2">
              <Slider
...
export default function CategoriaPage({ params }: { params: { slug: string } }) {
  ...
  // Adicionar o estado para o carrinho e feedback visual após a declaração dos outros estados
  ...
  // Adicionar useEffect para carregar o carrinho do localStorage antes do return
  useEffect(() => {
